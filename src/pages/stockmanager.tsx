...
function getPOItemDateTrackingType(
  item: POItem,
): "none" | "expiry" | "shelf_life" {
  const itemCategory = String(item.category ?? "");
  const { matchedCategory, dateTrackingType } =
    resolveInventoryCategoryMatch(itemCategory);
...
function NearestTimingCell({ info }: { info: NearestTimingInfo }) {
  const toneClass =
    info.status === "expired"
      ? "text-red-600"
      : info.status === "near"
...
function CreatePOModal({
  ...
  const handleSubmit = async () => {
    ...
    const invalidNameItem = items.find((item) => {
      ...
      return (
        !trimmedName || trimmedName.length > PURCHASE_ORDER_NAME_MAX_LENGTH
      );
    });
    if (invalidNameItem) {
      onShowToast(
    ...
    const missingUnitItem = items.find(
      ...
      {insufficient && (
        <div className="px-3.5 py-2.5 bg-red-50 border-t border-red-100">
          <p className="text-xs font-semibold text-red-600">
            {"\u26A0"} Insufficient stock {"\u2014"} need {qtyNeeded} {unit} but
            only {totalAvailable} {unit} available
          </p>
        </div>
      )}
...
export default function StockManager() {
  ...
  const setWdProductId = useCallback(
    (value: number | null) => {
  ...
  const dashboardFilteredProducts = useMemo(() => {
    ...
    const base = products.filter(
      (p) => !isMenuFoodProduct(p) && isMainStockDashboardCategory(p.category),
    );
    const filtered = !q
      ? base
      : base.filter(
                    ...
                    {(wholeChickenProducts.length > 0 ||
                      ...
                      >
                        ...
                        <p className="text-xs text-orange-400 italic">
                          Delivered whole {"\u2192"} chopped separately in
                          inventory
                        </p>
                      </motion.div>
                    )}
                  ...
                  >
                          ...
                          <div className="p-5 space-y-4">
                              ...
                              selectedWithdrawalStatus !== "normal" && (
                                ...
                                >
                                  {selectedWithdrawalStatus === "critical"
                                    ...
                                    : "Low stock warning"}
                                  : {selectedWithdrawalProduct.product_name} is
                                  at {selectedWithdrawalPct}% of reorder level (
                                  {selectedWithdrawalProduct.mainStock}{" "}
                            ...
                            {selectedWithdrawalProduct?.category
                              ...
                              <div className="text-xs px-3 py-2 rounded-xl border bg-rose-50 text-rose-500 border-rose-100">
                                {"\u26A0\uFE0F"} Sauce items are not reconciled
                                at end-of-day. Once withdrawn, they are
                                considered consumed.
                              </div>
                            )}
                            {false && (
                    ...
                    {withdrawalSubTab === "currently-withdrawn" && (
                      <motion.div variants={itemVariants}>
                        ...
                        >
                          {products.filter((p) => p.dailyWithdrawn > 0)
                            .length === 0 ? (
                        ...
                        <p className="text-3xl font-bold text-green-600 mt-1">
                          {
                            products.filter(
                              (p) =>
                                !isMenuFoodProduct(p) &&
                                getAlertSeverity(p, stockAlertSettings) ===
                                  "normal",
                            ).length
                          }
                        </p>
              ...
              >
                <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      Returns are saved on the batch {"\u2014"} staff will see
