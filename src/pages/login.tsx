...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // TEMPORARY BYPASS - remove when backend is ready
    if (formData.email === "devtest@gmail.com" && formData.password === "dev123") {
    ...
    if (isLogin) {
      try {
        ...
        setError(err.message || "Invalid credentials. Use dev@test.com / dev123 for dev access.");
      } finally {
        setIsLoading(false);
      }
    } else {
      ...
      try {
        ...
        setError(err.message || "Failed to register. Use dev@test.com / dev123 for dev access.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 p-4 font-['Poppins',sans-serif]">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8">
        ...
        {/* DEV BYPASS HINT */}
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-300 rounded-lg text-yellow-700 text-xs text-center">
          🛠 Dev bypass: <strong>devtest@gmail.com</strong> / <strong>dev123</strong>
        </div>
