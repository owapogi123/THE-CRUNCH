import React, { useState, useEffect, useRef } from 'react'
  ...
  function OrderTypeModal({ onClose, onPickUp }: { onClose: () => void; onPickUp: () => void }) {
      ...
      <>
        <motion.div
          ...
          style={{
            ...
            zIndex: 500, backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)',
          }}
        />
        <motion.div
        ...
        >
          <motion.button
            ...
            style={{
              ...
              background: '#1e1b17', border: '1px solid rgba(240,237,232,0.14)',
              color: 'rgba(240,237,232,0.55)', cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 18, fontWeight: 700, zIndex: 10, fontFamily: 'inherit',
            }}
          >×</motion.button>
            ...
            {view === 'choose' && (
              ...
              >
                <motion.button
                  ...
                  onClick={onPickUp}
                  style={{
                    background: '#151210', border: '1px solid rgba(245,200,66,0.18)',
                    borderRadius: 24, padding: '36px 32px 32px', cursor: 'pointer',
                ...
                >
                  ...
                  <p style={{ fontSize: 11, fontWeight: 700, color: '#f5c842', letterSpacing: '0.2em', textTransform: 'uppercase', margin: '0 0 8px' }}>Pick-up Order</p>
                  <h3 style={{ fontSize: 22, fontWeight: 900, color: '#f0ede8', margin: '0 0 14px', letterSpacing: '-0.02em', lineHeight: 1.15, fontFamily: "'Poppins', sans-serif" }}>Order here,<br />pick up fresh.</h3>
                  <div style={{ height: 1, background: 'rgba(240,237,232,0.07)', margin: '0 0 16px' }} />
                  <p style={{ fontSize: 13, fontWeight: 400, color: 'rgba(240,237,232,0.42)', lineHeight: 1.7, margin: '0 0 auto' }}>Order now and collect your items in-store.</p>
                  <div style={{ marginTop: 28, padding: '12px 0', borderRadius: 12, background: '#f5c842', textAlign: 'center', fontSize: 14, fontWeight: 800, color: '#111', letterSpacing: '0.02em' }}>Browse the Menu</div>
                </motion.button>
                ...
                <motion.button
                  ...
                  style={{
                    background: '#151210', border: '1px solid rgba(240,237,232,0.09)',
                    borderRadius: 24, padding: '36px 32px 32px', cursor: 'pointer',
                    fontFamily: "'Poppins', sans-serif", textAlign: 'left',
                    display: 'flex', flexDirection: 'column', gap: 0,
                    position: 'relative', overflow: 'hidden', transition: 'border-color 0.25s',
                ...
                >
                  ...
                  <div style={{ width: 56, height: 56, borderRadius: 16, background: 'rgba(240,237,232,0.06)', border: '1px solid rgba(240,237,232,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(240,237,232,0.6)', marginBottom: 22 }}>
                    <IconScooter />
                  </div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(240,237,232,0.35)', letterSpacing: '0.2em', textTransform: 'uppercase', margin: '0 0 8px' }}>Delivery Order</p>
                  <div style={{ height: 1, background: 'rgba(240,237,232,0.07)', margin: '0 0 16px' }} />
                  <p style={{ fontSize: 13, fontWeight: 400, color: 'rgba(240,237,232,0.42)', lineHeight: 1.7, margin: '0 0 auto' }}>Have your order delivered via a third-party service. You will be redirected to Foodpanda or Grab to complete the delivery.</p>
                  <div style={{ display: 'flex', gap: 8, marginTop: 24, marginBottom: 28 }}>
                    {[
                      ...
                      <span key={p.name} style={{ fontSize: 11, fontWeight: 700, padding: '5px 12px', borderRadius: 20, background: p.bg, color: p.color, border: `1px solid ${p.border}`, letterSpacing: '0.04em' }}>{p.name}</span>
                    ))}
                  </div>
                  <div style={{ padding: '12px 0', borderRadius: 12, background: 'rgba(240,237,232,0.07)', border: '1px solid rgba(240,237,232,0.12)', textAlign: 'center', fontSize: 14, fontWeight: 700, color: 'rgba(240,237,232,0.7)', letterSpacing: '0.02em' }}>Order via Delivery App</div>
                </motion.button>
              </motion.div>
            )}
            ...
            {view === 'delivery' && (
              ...
              >
                ...
                <motion.button whileHover={{ x: -2 }} whileTap={{ scale: 0.95 }} transition={SP} onClick={() => setView('choose')}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Poppins', sans-serif", fontSize: 12, fontWeight: 600, color: 'rgba(240,237,232,0.35)', padding: 0, marginBottom: 24, display: 'flex', alignItems: 'center', gap: 6, letterSpacing: '0.04em' }}>
                  ← Back
                </motion.button>
                <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(240,237,232,0.35)', letterSpacing: '0.2em', textTransform: 'uppercase', margin: '0 0 8px' }}>Delivery Order</p>
                <h3 style={{ fontSize: 24, fontWeight: 900, color: '#f0ede8', margin: '0 0 6px', letterSpacing: '-0.02em', fontFamily: "'Poppins', sans-serif" }}>Select a delivery app to continue</h3>
                <p style={{ fontSize: 13, color: 'rgba(240,237,232,0.38)', margin: '0 0 28px', fontWeight: 300 }}>Clicking the link will open the app or website in a new tab.</p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <motion.a href={DELIVERY_LINKS.foodpanda} target="_blank" rel="noopener noreferrer"
                    whileHover={{ borderColor: 'rgba(233,30,140,0.45)', y: -3 }} whileTap={{ scale: 0.97 }} transition={SPG}
                    style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 0, background: 'rgba(233,30,140,0.05)', border: '1px solid rgba(233,30,140,0.2)', borderRadius: 18, padding: '26px 24px 22px', textDecoration: 'none', position: 'relative', overflow: 'hidden', transition: 'border-color 0.25s', cursor: 'pointer' }}>
                    <div style={{ position: 'absolute', top: 0, left: 12, right: 12, height: 1, background: 'linear-gradient(90deg, transparent, rgba(233,30,140,0.3), transparent)' }} />
                    <p style={{ fontSize: 17, fontWeight: 800, color: '#f0ede8', margin: '0 0 5px', letterSpacing: '-0.01em', fontFamily: "'Poppins', sans-serif" }}>FoodPanda</p>
                    <p style={{ fontSize: 12, color: 'rgba(240,237,232,0.38)', margin: '0 0 18px', fontWeight: 300, lineHeight: 1.5 }}>Order via the FoodPanda app or website</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: '#e91e8c', padding: '8px 14px', borderRadius: 20, background: 'rgba(233,30,140,0.1)', border: '1px solid rgba(233,30,140,0.2)' }}>Open FoodPanda <IconExternalLink /></div>
                  </motion.a>
                  <motion.a href={DELIVERY_LINKS.grab} target="_blank" rel="noopener noreferrer"
                    whileHover={{ borderColor: 'rgba(0,177,79,0.45)', y: -3 }} whileTap={{ scale: 0.97 }} transition={SPG}
                    style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 0, background: 'rgba(0,177,79,0.04)', border: '1px solid rgba(0,177,79,0.18)', borderRadius: 18, padding: '26px 24px 22px', textDecoration: 'none', position: 'relative', overflow: 'hidden', transition: 'border-color 0.25s', cursor: 'pointer' }}>
                    <div style={{ position: 'absolute', top: 0, left: 12, right: 12, height: 1, background: 'linear-gradient(90deg, transparent, rgba(0,177,79,0.28), transparent)' }} />
                    <p style={{ fontSize: 17, fontWeight: 800, color: '#f0ede8', margin: '0 0 5px', letterSpacing: '-0.01em', fontFamily: "'Poppins', sans-serif" }}>Grab</p>
                    <p style={{ fontSize: 12, color: 'rgba(240,237,232,0.38)', margin: '0 0 18px', fontWeight: 300, lineHeight: 1.5 }}>Order via the Grab app or website</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: '#00b14f', padding: '8px 14px', borderRadius: 20, background: 'rgba(0,177,79,0.09)', border: '1px solid rgba(0,177,79,0.18)' }}>Open Grab <IconExternalLink /></div>
                  </motion.a>
                </div>
              </motion.div>
            )}
