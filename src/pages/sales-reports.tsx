...
import { ChevronLeft, ChevronRight, CalendarDays, X, RotateCcw } from "lucide-react";
...
function filterOrdersByRange(orders: Order[], from: Date | null, to: Date | null): Order[] {
...
function RefundModal({ open, log, onConfirm, onClose, loading }: RefundModalProps) {
  return (
    <AnimatePresence>
      {open && log && (
        <>
          ...
          <motion.div
            ...
            style={{
              position: "fixed", top: "29%", left: "37%",
              transform: "translate(-50%, -50%)",
              zIndex: 301, width: 400, background: "#fff",
          ...
          >
            ...
            <div style={{ padding: "24px 24px 20px" }}>
              ...
              <p style={{ margin: "0 0 20px", fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
                Are you sure you want to refund <strong>₱{log.total.toLocaleString()}</strong> for{" "}
                <strong>{log.product}</strong>? The order status will be updated to{" "}
...
function DrumCol({ label, items, selectedIndex, onChange }: DrumColProps) {
  ...
  const state = useRef({
    curY: -selectedIndex * ITEM_H, targetIdx: selectedIndex,
    vel: 0, startY: 0, startCurY: 0,
    dragging: false, lastY: 0, lastT: 0,
  });

  function clampIdx(i: number) { return Math.max(0, Math.min(items.length - 1, Math.round(i))); }
  ...
  useEffect(() => {
    function onMouseMove(e: MouseEvent) {
      ...
      const now = Date.now(); const dt = Math.max(1, now - s.lastT);
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
      <p style={{
        ...
        color: "#94a3b8", marginBottom: 8, fontFamily: "'Poppins', sans-serif",
      }}>{label}</p>
...
function DrumDatePicker({ open, title, initial, onApply, onClose }: DrumDatePickerProps) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <>
          <motion.div key="backdrop" initial={{ opacity: 0, backdropFilter: "blur(0px)" }} animate={{ opacity: 1, backdropFilter: "blur(6px)" }} exit={{ opacity: 0, backdropFilter: "blur(0px)" }} transition={{ duration: 0.28 }} onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.35)" }} />
          <motion.div key="sheet" initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} transition={{ duration: 0.28, ease: "easeOut" }} style={{ position: "absolute", top: "30%", left: "35%", transform: "translate(-50%, -50%)", zIndex: 201, width: 420, background: "#fff", borderRadius: 20, border: "0.5px solid #e2e8f0", paddingBottom: 32, fontFamily: "'Poppins', sans-serif", boxShadow: "0 10px 40px rgba(0,0,0,0.18)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px 14px", borderBottom: "0.5px solid #f1f5f9" }}>
              ...
              <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: "50%", border: "0.5px solid #e2e8f0", background: "#f8fafc", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: "#94a3b8", lineHeight: 1 }}>×</button>
            </div>
...
function RevenueDropdown({ period, setPeriod, logs }: { period: Period; setPeriod: (p: Period) => void; logs: SaleLog[] }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", minWidth: 280 }}>
      <motion.div whileHover={{ boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }} onClick={() => setOpen(!open)}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <motion.span animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }} style={{ color: "#94a3b8", fontSize: 10, display: "inline-block", lineHeight: 1 }}>▼</motion.span>
          </div>
        </div>
        <AnimatePresence mode="wait">
          <motion.p key={`${period}-${revenue}`} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: 0.18 }}
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.97 }} transition={{ duration: 0.18, ease: "easeOut" }}
            style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, boxShadow: "0 8px 30px rgba(0,0,0,0.1)", overflow: "hidden", zIndex: 50, minWidth: "100%" }}>
            {periods.map((p) => (
              <motion.div key={p} whileHover={{ background: "#f8fafc" }} onClick={() => { setPeriod(p); setOpen(false); }}
                style={{ padding: "11px 18px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #f8fafc" }}>
                <span style={{ color: period === p ? "#0f172a" : "#64748b", fontSize: 13, fontWeight: period === p ? 600 : 400, fontFamily: "'Poppins', sans-serif" }}>{p}</span>
...
function LogRow({ log, index, onRefund }: LogRowProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03, duration: 0.28, ease: "easeOut" }}>
      <motion.div onClick={() => setOpen(!open)} whileHover={{ backgroundColor: "#fafafa" }}
        ...
        <span style={{ color: log.status === "Cancelled" || log.status === "Refunded" ? "#dc2626" : "#0f172a", fontSize: 14, fontWeight: 600, width: 100, textAlign: "right", flexShrink: 0, textDecoration: log.status === "Cancelled" ? "line-through" : "none" }}>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div style={{ padding: "14px 20px 14px 42px", display: "flex", gap: 32, flexWrap: "wrap", alignItems: "flex-end", justifyContent: "space-between" }}>
              {/* Detail fields */}
              <div style={{ display: "flex", gap: 32, flexWrap: "wrap" }}>
                {[
                  ...
                  <div key={f.label}>
                    ...
                    <p style={{
                      color: f.label === "Order Status" ? statusColor[log.status as Status]
                        : f.label === "Cashier" ? "#4A1C1C" : "#334155",
                      fontSize: 13,
              ...
              {canRefund && (
                <motion.button
                  whileHover={{ scale: 1.03, boxShadow: "0 4px 16px rgba(220,38,38,0.18)" }}
                  whileTap={{ scale: 0.97 }}
                  onClick={(e) => { e.stopPropagation(); onRefund(log); }}
                  style={{
...
function SummaryBar({ logs }: { logs: SaleLog[] }) {
  ...
  return (
    <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
      {stats.map((s) => (
        <motion.div key={s.label} whileHover={{ y: -2, boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }}
...
function OrdersTab({ orders }: { orders: Order[] }) {
  ...
  function handlePickerApply(date: Date) {
    ...
    setActiveQuick(null); setCurrentPage(1); setPickerOpen(false);
  }

  function clearRange() { setFromDate(null); setToDate(null); setActiveQuick("all"); setCurrentPage(1); }
  ...
  const statusBadgeClass = (s: string) =>
    s === "Completed" ? "bg-green-50 text-green-700 hover:bg-green-50 rounded-lg font-medium border-0"
      : s === "Pending" ? "bg-yellow-50 text-yellow-700 hover:bg-yellow-50 rounded-lg font-medium border-0"
        : "bg-red-50 text-red-700 hover:bg-red-50 rounded-lg font-medium border-0";

  const orderTypeBadgeClass = (t: string) =>
    t === "take-out" ? "bg-amber-50 text-amber-700 hover:bg-amber-50 rounded-lg font-medium border-0"
      : t === "delivery" ? "bg-blue-50 text-blue-700 hover:bg-blue-50 rounded-lg font-medium border-0"
        : "bg-rose-50 text-rose-700 hover:bg-rose-50 rounded-lg font-medium border-0";

  const fmtDate = (v?: string | null) => { const d = parseDateSafe(v); return !d ? "-" : d.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }); };
  const fmtTime = (v?: string | null) => { const d = parseDateSafe(v); return !d ? "-" : d.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: true }); };

  return (
    <>
      <DrumDatePicker open={pickerOpen} title={pickerTarget === "from" ? "Select from date" : "Select to date"} initial={pickerInitial} onApply={handlePickerApply} onClose={() => setPickerOpen(false)} />
      ...
      <Card className="bg-white rounded-2xl p-6 shadow-md border-0">
        <div className="flex items-start justify-between mb-4 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => openDatePicker("from")} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-sm font-medium transition-all ${fromDate ? "border-[#4A1C1C] text-[#4A1C1C] bg-[#4A1C1C]/5" : "border-gray-200 text-gray-500 hover:border-gray-300 hover:text-gray-700 bg-white"}`}>
              {fromDate ? formatDisplayDate(fromDate) : "Select date"}
            </button>
            <span className="text-xs text-gray-400">to</span>
            <button onClick={() => openDatePicker("to")} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-sm font-medium transition-all ${toDate ? "border-[#4A1C1C] text-[#4A1C1C] bg-[#4A1C1C]/5" : "border-gray-200 text-gray-500 hover:border-gray-300 hover:text-gray-700 bg-white"}`}>
            ...
            {filtered.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center text-gray-400 py-10">
                  {orders.length === 0 ? "No orders yet. Orders will appear here once the cashier processes them." : "No orders found for the selected date range."}
                </TableCell>
              </TableRow>
            ) : (
              paginated.map((order) => (
                <TableRow key={order.id} className="border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <TableCell><Badge variant="secondary" className={orderTypeBadgeClass(order.orderType)}>{order.orderType || "-"}</Badge></TableCell>
                  <TableCell><Badge variant="secondary" className={statusBadgeClass(order.status)}>{order.status}</Badge></TableCell>
...
export default function SalesReports() {
  ...
  const fetchSalesData = async () => {
    try {
      ...
      rows.forEach((r) => {
        ...
        const paymentMethod =
          (r.paymentMethod ?? r.payment_method ?? "cash").toString().trim() || "cash";

        // Resolve cashier name from whichever field the backend exposes
        const cashierName =
          (r.cashierName ?? r.cashier_name ?? r.operatorName ?? r.operator_name ?? "")
            .toString().trim() || "Cashier";
      ...
      setLogs(mappedLogs.sort((a, b) => b._dateObj.getTime() - a._dateObj.getTime()));
  ...
  async function handleRefundConfirm() {
    ...
    try {
      await api.patch(`/orders/${refundTarget.orderId}/status`, { status: "refunded" });
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15, duration: 0.35 }}
              ...
              <input
                ...
                style={{
                  ...
                  outline: "none", fontFamily: "'Poppins', sans-serif", boxShadow: "0 1px 3px rgba(0,0,0,0.04)",
                }}
              />
                  ...
                  dates.map((date) => {
                    ...
                    return (
                      <div key={date}>
                        ...
                        {entries.map((log, i) => (
                          <LogRow key={log.id} log={log} index={i} onRefund={(l) => setRefundTarget(l)} />
                        ))}
                      </div>
                    );
                  })
