    ...
    <div className="sm-overlay" onClick={onClose}>
      <div className="sm-modal" onClick={(e) => e.stopPropagation()}>
        <div className="sm-mhead">
          ...
          <button className="sm-xbtn" onClick={onClose}>
            ×
          </button>
        </div>
...
interface SMFIProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label: string;
}
function SMFI({ label, ...rest }: SMFIProps) {
  return (
    <SMFG label={label}>
      <input className="sm-finput" {...rest} />
    </SMFG>
  );
}
            ...
            <tr>
              <td colSpan={cols.length}>
                <div style={{ textAlign: "center", padding: "42px 0" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#c4c4c4", marginTop: 3 }}>
                    {emptyHint}
                  </div>
                </div>
              </td>
...
function SMStatCard({
  ...
  return (
    <div className="sm-stat-card" style={{ borderTop: `3px solid ${c}` }}>
      ...
      <div style={{ fontSize: 24, fontWeight: 800, lineHeight: 1, color: c }}>
        {value}
      </div>
      {meta && (
    ...
    <div>
      ...
      <div className="sm-filter-row">
        ...
        {["All", "Pending", "Received", "Cancelled"].map((f) => (
          <button
            key={f}
            className={`sm-chip ${filter === f ? "sm-chip-on" : ""}`}
            onClick={() => setFilter(f)}
          >
            {f}
          </button>
      ...
      <SMTblWrap
        ...
        rows={filtered.map((po) => (
          <tr key={po.id}>
            ...
            <td>
              <span className={sBadgeClass(po.status)}>{po.status}</span>
            </td>
            <td>
              <div style={{ display: "flex", gap: 5 }}>
                <button
                  className="sm-abtn sm-ghost"
        ...
        <SMModal
          ...
          footer={
            <>
              ...
              <button className="sm-abtn sm-primary" onClick={submitPO}>
                Submit PO
              </button>
            </>
          }
        >
          ...
          {poItems.map((item, i) => (
            ...
            >
              ...
              />
              <button className="sm-rm-btn" onClick={() => removePoItem(i)}>
                ×
              </button>
            </div>
          ))}
          <button className="sm-add-row-btn" onClick={addPoItem}>
            + Add another item
          </button>
        ...
        >
          <div className="sm-detail-grid">
            {(
              ...
              <div key={k} className="sm-detail-cell">
                <div className="sm-detail-key">{k}</div>
                <div className="sm-detail-val">{v}</div>
              </div>
            ))}
          </div>
          <label
      ...
      <SMTblWrap
        ...
        rows={filtered.map((r) => (
          <tr key={r.id}>
            ...
            <td
              style={{
                color: r.poRef ? "#4f46e5" : "#9ca3af",
                fontWeight: r.poRef ? 600 : 400,
              }}
            >
              {r.poRef || "—"}
            </td>
        ...
        <SMModal
          ...
          footer={
            <>
              ...
              <button className="sm-abtn sm-primary" onClick={submitSI}>
                Save Record
              </button>
            </>
          }
...
function StockTransfer() {
  ...
  function submitTR() {
    ...
    if (trFrom === trTo)
      return alert("Source and destination must be different.");
    setTransfers((p) => [
      {
      ...
      <SMTblWrap
        ...
        rows={filtered.map((t) => (
          <tr key={t.id}>
            ...
            <td>{t.date}</td>
            <td>
              <span className={sBadgeClass(t.status)}>{t.status}</span>
            </td>
            <td style={{ color: t.approvedBy === "—" ? "#9ca3af" : "#374151" }}>
              {t.approvedBy}
            </td>
            <td>
              {t.status === "Pending" && (
        ...
        <SMModal
          ...
          footer={
            <>
              ...
              <button className="sm-abtn sm-primary" onClick={submitTR}>
                Submit Transfer
              </button>
            </>
          }
      ...
      <SMTblWrap
        ...
        rows={filtered.map((a) => (
          <tr key={a.id}>
            ...
            <td>
              <span className="sm-badge sm-yellow">{a.reason}</span>
            </td>
            <td>{a.date}</td>
            <td>{a.by}</td>
          </tr>
        ...
        <SMModal
          ...
          footer={
            <>
              ...
              <button className="sm-abtn sm-primary" onClick={submitAdj}>
                Save Adjustment
              </button>
            </>
          }
      ...
      <SMTblWrap
        ...
        rows={filtered.map((l) => (
          <tr key={l.id}>
            ...
            <td>
              <span className={tBadgeClass(l.type)}>{l.type}</span>
            </td>
            <td style={{ fontWeight: 500 }}>{l.item}</td>
            <td
              style={{
                fontWeight: 700,
                color: l.qty.startsWith("+") ? "#16a34a" : "#dc2626",
              }}
            >
              {l.qty}
            </td>
  ...
  const loadInventory = async (showLoader = true) => {
    try {
      ...
      if (data && Array.isArray(data)) {
        ...
        const normalizedRows = Array.from(groupedByName.values()).map(
          (group) => {
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
      ...
      <div className="sm-tabs-bar">
        {SM_TABS.map((tab) => (
          <button
            key={tab.key}
            className={`sm-tab ${activeSmTab === tab.key ? "sm-tab-active" : ""}`}
