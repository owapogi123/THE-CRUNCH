...
interface UseBatchInventoryReturn {
  ...
  addBatch: (itemId: number, quantity: number, unit: string, expiryDate?: Date) => void
  removeBatch: (itemId: number, batchId: string) => void
  consumeProduct: (itemId: number, quantity: number) => { success: boolean; message: string; affected: any[] }
  returnBatchQuantity: (itemId: number, batchId: string, quantity: number) => void
  getTotalBatchQuantity: (itemId: number) => number
  getOldestBatch: (itemId: number) => Batch | null
  updateBatchStatus: (itemId: number, batchId: string, status: 'active' | 'partial' | 'returned') => void
 ...
 */
export function useBatchInventory(initialItems: InventoryItemWithBatches[]): UseBatchInventoryReturn {
  ...
  const consumeProduct = useCallback((itemId: number, quantity: number) => {
    ...
    setItems(prev =>
      prev.map(item => {
        ...
        const updatedBatches = item.batches?.map(batch => {
          const affected = result.batchesAffected.find(a => a.batchId === batch.id)
          if (affected) {
    ...
    return {
      success,
      message: success ? 'Product consumed successfully' : 'Insufficient batch quantity',
   ...
   */
  const returnBatchQuantity = useCallback((itemId: number, batchId: string, quantity: number) => {
    setItems(prev =>
      prev.map(item =>
        item.id === itemId
          ? {
   ...
   */
  const getTotalBatchQuantity = useCallback((itemId: number): number => {
   ...
   */
  const getOldestBatch = useCallback((itemId: number): Batch | null => {
    const item = items.find(i => i.id === itemId)
        ...
        item.id === itemId
          ? {
              ...
              batches: item.batches?.filter(batch => {
                const isExpired = batch.expiresAt && new Date() > batch.expiresAt
